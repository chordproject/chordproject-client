...
import { Observable, from, throwError, of, combineLatest } from 'rxjs';
import { map, catchError, switchMap } from 'rxjs/operators';
import { MatSnackBar } from '@angular/material/snack-bar';
import { Auth } from 'firebase/auth';
import { SongService, PartialSong } from './song.service';
...
export class SongbookService {
    ...
    /**
     * Get a songbook by ID
     */
    get(id: string): Observable<Songbook> {
        return from(getDoc(doc(this._firestore, 'songbooks', id)))
            .pipe(
                map(docSnap => {
                    if (docSnap.exists()) {
    ...
    /**
     * Get content of a songbook (songs)
     */
    getContent(songbookId: string): Observable<PartialSong[]> {
        ...
        return from(getDocs(q)).pipe(
            switchMap(relationsSnapshot => {
                // Extract song IDs and ordering information
    ...
    async save(songbook: Songbook): Promise<string> {
        ...
        try {
            ...
            if (!songbook.uid) {
                ...
                songbook.source = 'homenajesus'; // Set appropriate source
            }

            songbook.authorId = user.uid;

            await setDoc(doc(this._firestore, 'songbooks', songbook.uid), { ...songbook });
     ...
     */
    async addSong(songbookId: string, songId: string, order?: number): Promise<string> {
        if (!this.verifyAuthentication()) {
            return null;
        }
        ...
        try {
            ...
            await setDoc(doc(this._firestore, 'songbook_songs', relationId), relation);
    ...
    async removeSong(songbookId: string, songId: string): Promise<boolean> {
        ...
        try {
            ...
            await setDoc(doc(this._firestore, 'songbook_songs', relationDoc.id), {
                deleted: true,
